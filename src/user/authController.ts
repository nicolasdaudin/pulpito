...
// FIXME: dependance to mongoose - Types.ObjectId.
...
// TODO: remove dependance to mongoose - HydratedDocument.
// TODO: this method does many things: clears password from output, sign token, prepares the answer, sets the cookie...
...
const login = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user || !(await user.isCorrectPassword(password, user.password))) {
      return next(
        new AppError(
          'Incorrect email or password, or user no longer active',
          401
        )
      );
    }
...
const protect = catchAsync(
  async (
    ...
    // FIXME: use UserRepository instead of directly User
